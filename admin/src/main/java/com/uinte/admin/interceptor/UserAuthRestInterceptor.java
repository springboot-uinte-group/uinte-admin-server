package com.uinte.admin.interceptor;

import com.alibaba.fastjson.JSONObject;
import com.google.common.base.Predicate;
import com.google.common.collect.Collections2;
import com.uinte.admin.annotation.IgnoreUserToken;
...
public class UserAuthRestInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		if (result.size() > 0) {
			...
			// 根据用户的id获取该用户所拥护的权限
			List<PermissionInfo> permissionInfos = permissionService
					.getPermissionByUsername(infoFromToken.getUniqueName());
			// 判断该用户所访问的url，该用户是否用户此权限
			if (this.getPermissionInfos(permissionInfos, requestUri, method).size() > 0) {
				// 如果有则放行
				handleUserInfo(infoFromToken, token);
				return super.preHandle(request, response, handler);
			}
//                否则 抛异常
...
//        不属于管控的url 直接放行
	...
	private void handleUserInfo(IJWTInfo infoFromToken, String token) {
		try {
	 ...
	 * @return
	 */
	private Collection<PermissionInfo> getPermissionInfos(List<PermissionInfo> serviceInfo, final String requestUri,
			final String method) {
